...
function showSlide(n) {
    for(let i=0; i <slides.length; i++) { // slide 배열의 처음부터 끝까지 반복
        slides[i].style.display = "none"; // 모든 이미지를 화면에서 감춤
    }
    slides[n].style.display = "block"; // n번째 이미지를 화면에 표시
}

function preySlide() { //왼쪽 버튼 눌렀을 때의 함수
    if(current > 0 ) current -= 1; // 현재 이미지가 첫번째가 아닐 경우 , -1 위치로
    else current = sliders.length -1; // 현재 이미지가 첫번째라면 마지막으로
    showSlide(current); // 이동한 위치의 이미지 표시
}

function nextSlide() { // 오른쪽 버튼 눌렀을 때의 함수
